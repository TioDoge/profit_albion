...
/* [MS-XLSB] 2.5.21 */
function parse_XLSBCodeName (data, length) { return parse_XLWideString(data); }
...
function parse_TypedPropertyValue(blob, type, _opts) {
	...
	switch(type === VT_VARIANT ? t : type) {
		...
		case 0x51 /*VT_USTR*/: return parse_VtUnalignedString(blob, t).replace(chr0,'');
...
//	while(blob.l < target) arr.push(cb(blob, target - blob.l));
	while(blob.l < target) arr.push(cb(blob));
...
//	else controlInfo = parse_ControlInfo(blob, 6, opts);
	else controlInfo = parse_ControlInfo(blob, 6);
	var cchText = blob.read_shift(2);
	var cbRuns = blob.read_shift(2);
//	var ifntEmpty = parse_FontIndex(blob, 2);
	var ifntEmpty = parse_FontIndex(blob);
...
function parse_zip(zip, opts) {
	...
	if(dir.workbooks.length === 0) {
		...
		if(!getzipfile(zip,binname)) throw new Error("Could not find workbook");
...
function sheet_to_json(sheet, opts){
	var val, row, range, header = 2, offset = 3, r, hdr = [], isempty, R, C, v;
